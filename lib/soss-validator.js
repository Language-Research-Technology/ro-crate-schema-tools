...
class SossValidator {
  ...
  validateProperty(entity, propertyRule){
    ...
    this.log(`<---------------------   Validating property ${propertyName} with values ${JSON.stringify(propertyValues)} for entity ${entityId}`, 'debug');

    // Check if property has a fixed value defined by schema:value
    if (propertyRule.value) {
      const expectedValue = propertyRule['value'];
      this.log(`Property ${propertyName} has a fixed expected value: ${expectedValue}`, 'debug');
      ...
      if (!propertyValues && propertyRule['sh:minCount'] && parseInt(propertyRule['sh:minCount']) > 0) {
        this.log(`Entity ${entityId} is missing required property ${propertyName} with fixed value ${expectedValue}`, 'error');
        this.addResult('error', `Entity ${entityId} is missing required property ${propertyName} with fixed value ${expectedValue}`);
        return false;
      }
      ...
      if (propertyValues) {
        ...
        for (const value of propertyValues) {
            this.log(`Checking value ${JSON.stringify(value)} against expected fixed value ${expectedValue[i]}`, 'debug');
            if  (value === expectedValue[i] && matches  === true) {
                this.log(`Entity ${entityId} property ${propertyName} matches expected fixed value ${expectedValue[i]}`, 'debug');
            } else {
                this.log(`Entity ${entityId} property ${propertyName} does not match expected fixed value ${expectedValue[i]}`, 'error');
                matches = false;
                continue
            } 
            i += 1;
        ...
        if (!matches) {
          this.log(`Entity ${entityId} property ${propertyName} values do not match expected fixed value ${expectedValue}`, 'error');
          this.addResult('error', `Entity ${entityId} property ${propertyName} values do not match expected fixed value ${expectedValue}`);
          return false;
        }
        
        this.log(`Entity ${entityId} property ${propertyName} matches fixed value ${expectedValue}`, 'debug');
        return true;
      }
