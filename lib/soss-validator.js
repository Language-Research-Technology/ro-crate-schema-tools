...
class SossValidator {
  ...
  validateTargetCrateGraph(targetCrate) {
    ...
    */
    ...
    for (const classRule of entitiesByType["rdfs:Class"] || []) {
      ...
      const types = classRule["prov:specializationOf"];
  ...
  validateEntityTypes(entity, requiredTypes) {
    ...
    const entityTypes = rawEntityTypes.map((type) => {
      console.log(`Resolving entity type ${type}`, "debug");
      ...
      if (typeof value === "object" && value["@id"]) {
        ...
        if (referencedEntity) {
          ...
          if (
            ...
            return this.validateAgainstItemList(
              value,
              referencedRule
            );
          } else {
          ...
          for (const prop in item) {
            ...
            for (const expectedVal of expectedValues) {
              ...
              for (const actVal of actualValues) {
                ...
                if (typeof expectedVal === 'object' && expectedVal['@id'] && 
                    typeof actVal === 'object' && actVal['@id']) {
              ...
              if (!found) {
                const expectedDisplay = typeof expectedVal === 'object' ? 
                  JSON.stringify(expectedVal) : expectedVal;
